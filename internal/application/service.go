...
type Service struct {
	...
	previews    map[string]domain.AuditRound
}

func NewService(repo *store.Repository, maxParallel int) *Service {
	return &Service{
		repo:        repo,
		coordinator: NewCoordinator(maxParallel),
		now:         time.Now,
		previews:    map[string]domain.AuditRound{},
	}
}
...
func (s *Service) recordFailure(batchID string, revision int64, meta Metadata, fp string, err error) *AppError {
	...
	if recordErr := s.repo.RecordFailure(batchID, revision, meta.RequestID, fp, appErrorStatus(appError), encoded, s.now().UTC()); recordErr != nil {
...
func (s *Service) Freeze(batchID string, command FreezeCommand) (*MutationResult, error) {
	return s.mutate(batchID, command.Metadata, "rubric.frozen", command, func(batch *domain.CorpusBatch) error { return batch.Freeze(command.FreezeInput, s.now()) })
}

func (s *Service) RegisterItem(batchID string, command RegisterItemCommand) (*MutationResult, error) {
	return s.mutate(batchID, command.Metadata, "item.registered", command, func(batch *domain.CorpusBatch) error { return batch.RegisterItem(command.RegisterItemInput, s.now()) })
}
...
func (s *Service) SubmitAnnotation(batchID string, command SubmitAnnotationCommand) (*MutationResult, error) {
	return s.mutate(batchID, command.Metadata, "annotation.submitted", command, func(batch *domain.CorpusBatch) error {
		return batch.SubmitAnnotation(command.SubmitAnnotationInput, s.now())
	})
}

func (s *Service) Resolve(batchID string, command ResolveCommand) (*MutationResult, error) {
	return s.mutate(batchID, command.Metadata, "disagreement.resolved", command, func(batch *domain.CorpusBatch) error { return batch.Resolve(command.ResolveInput, s.now()) })
}
...
func (s *Service) CompleteAudit(batchID string, command AuditCommand) (*MutationResult, error) {
	return s.mutate(batchID, command.Metadata, "audit.completed", command, func(batch *domain.CorpusBatch) error {
		...
		return batch.CompleteAudit(*round, command.Findings, command.AuditorID, s.now())
	})
}

func (s *Service) Correct(batchID string, command CorrectCommand) (*MutationResult, error) {
	return s.mutate(batchID, command.Metadata, "item.corrected", command, func(batch *domain.CorpusBatch) error { return batch.Correct(command.CorrectInput, s.now()) })
}
