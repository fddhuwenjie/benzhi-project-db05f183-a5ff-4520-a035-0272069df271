...
func (s *Service) AuditPreview(batchID, seed string) (*domain.AuditRound, error) {
	cacheKey := batchID + "\x00" + seed
	s.previewMu.Lock()
	cached, ok := s.previews[cacheKey]
	s.previewMu.Unlock()
	if ok {
		return cloneAuditPreview(cached), nil
	}
	batch, err := s.repo.Load(batchID)
	if err != nil {
		return nil, classify(err)
	...
	s.previewMu.Lock()
	s.previews[cacheKey] = *cloneAuditPreview(*round)
	s.previewMu.Unlock()
	return round, nil
}

func cloneAuditPreview(round domain.AuditRound) *domain.AuditRound {
	cloned := round
	cloned.SampleItemIDs = append([]string(nil), round.SampleItemIDs...)
	cloned.Findings = append([]domain.AuditFinding(nil), round.Findings...)
	cloned.CorrectionItemIDs = append([]string(nil), round.CorrectionItemIDs...)
	return &cloned
}
