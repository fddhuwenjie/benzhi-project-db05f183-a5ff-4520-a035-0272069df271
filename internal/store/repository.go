...
var repositoryLockRegistry = struct {
	sync.Mutex
	locks map[string]*sync.RWMutex
}{locks: map[string]*sync.RWMutex{}}

func lockForRoot(root string) *sync.RWMutex {
	repositoryLockRegistry.Lock()
	defer repositoryLockRegistry.Unlock()
	if lock := repositoryLockRegistry.locks[root]; lock != nil {
		return lock
	}
	lock := &sync.RWMutex{}
	repositoryLockRegistry.locks[root] = lock
	return lock
}
...
func Open(root string) (*Repository, error) {
	...
	repo := &Repository{root: root, mu: lockForRoot(root), quarantined: map[string]string{}}
